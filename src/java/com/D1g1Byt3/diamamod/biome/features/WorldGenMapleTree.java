...
public class WorldGenMapleTree extends WorldGenAbstractTree {
	    ...
	    public WorldGenMapleTree(Block wood, Block leaves, int metaWood, int metaLeaves)
	    {
	        this(wood, leaves, metaWood, metaLeaves, false, 4, 3, false);
	    }

	    public WorldGenMapleTree(Block wood, Block leaves, int mW, int mL, boolean doBlockNotify, int minTH, int ranTH, boolean vG)
	    {
	        super(doBlockNotify);
	        this.leaves = leaves;
	        this.wood = wood;
	        this.randomTreeHeight = ranTH;
	    ...
	    public boolean generate(World world, Random random, int x, int y, int z)
	    {
	        ...
	        if (y >= 1 && y + l + 1 <= 256)
	        {
	            ...
	            else
	            {
	                ...
	                boolean isSoil = block2.canSustainPlant(world, x, y - 1, z, ForgeDirection.UP, (BlockSapling)ModBlocks.blockSapling);
	                if (isSoil && y < 256 - l - 1)
	                {
	                        ...
	                        for (i2 = x - l1; i2 <= x + l1; ++i2)
	                        {
	                            ...
	                            for (int k2 = z - l1; k2 <= z + l1; ++k2)
	                            {
	                                ...
	                                if (Math.abs(j2) != l1 || Math.abs(l2) != l1 || random.nextInt(2) != 0 && i3 != 0)
	                                {
	                                    ...
	                                    {
	                                        this.setBlockAndNotifyAdequately(world, i2, k1, k2, ModBlocks.blockLeaf, this.metaLeaves);
	                                    }
	                                }
	                            }
	                        }
